...
public class ConsentServiceImpl implements ConsentService {
    ...
    private List<TkmService> getRequestedTkmServices(String[] services){
        ...
        try {
            servicesEnums = Arrays.stream(Optional.ofNullable(services)
                    .orElse(new String[0]))
    ...
    private List<ConsentResponse> addDetail(TkmCard tkmCard, List<TkmService> tkmServices, List<ConsentResponse> details){
        ...
        serviceByConsent = cardServices.stream().filter(b-> b.getConsentType().equals(ConsentEntityEnum.Allow)).collect(Collectors.toList());
        if (CollectionUtils.isEmpty(serviceByConsent)) {
            return details;
        }
        Consent consent = new Consent();
        createConsentFromTkmCard(consent, serviceByConsent, tkmCard.getHpan());
        details.add(new ConsentResponse(consent));
        return details;
    }

    private void createConsentFromTkmCard(Consent consent, List<TkmCardService> services, String hpan) {
